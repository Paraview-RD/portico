...
-- What each application receives, and under what name.
--
-- Until now the answer was written into three functions — oidcp's
-- privateClaims, samlp's attributes, casp's writeSuccess — and was the same for
-- every application. That is a reasonable default and a poor rule: a service
-- provider maps by the name it is given, so an integration whose expected name
-- differs from Portico's had no way in but a code change, and the
-- twenty-five profile attributes had no way out at all.
--
-- One table for all three protocols rather than a column on each of their
-- registration tables. The mapping logic is one thing; three copies of it would
...
-- first went out of step. It also makes "which applications receive department"
...
CREATE TABLE application_field_mappings (
    ...
    -- Exactly one of these is set. Three nullable foreign keys rather than a
    -- kind column and a loose id, because this way the database enforces that
    -- the application exists and takes the mappings with it when it goes; a
    ...
    CONSTRAINT ck_application_field_mappings_one_application CHECK (
        (oauth_client_id IS NOT NULL)::int
        + (saml_sp_id IS NOT NULL)::int
        + (cas_service_id IS NOT NULL)::int = 1
    ),
    ...
    -- The name the application expects. For SAML this is the Name, which is
    ...
    -- One row per source per application. Renaming a fact twice for the same
    -- audience is not a thing anybody means to do, and if it were possible the
    -- winner would be whichever row was read first.
    CONSTRAINT uq_application_field_mappings_oauth
        UNIQUE (oauth_client_id, source_key),
    CONSTRAINT uq_application_field_mappings_saml
        UNIQUE (saml_sp_id, source_key),
    CONSTRAINT uq_application_field_mappings_cas
        UNIQUE (cas_service_id, source_key)
);

CREATE INDEX idx_application_field_mappings_oauth
    ON application_field_mappings (tenant_id, oauth_client_id)
    WHERE oauth_client_id IS NOT NULL;
CREATE INDEX idx_application_field_mappings_saml
    ON application_field_mappings (tenant_id, saml_sp_id)
    WHERE saml_sp_id IS NOT NULL;
CREATE INDEX idx_application_field_mappings_cas
    ON application_field_mappings (tenant_id, cas_service_id)
    WHERE cas_service_id IS NOT NULL;

-- Answers "which applications receive this fact", across all three protocols
-- at once. That is the disclosure question, and it is the reason this is one
-- table.
CREATE INDEX idx_application_field_mappings_source
    ON application_field_mappings (tenant_id, source_key);

COMMENT ON TABLE application_field_mappings IS
...
-- decided something — pre-filling ten per application would destroy the only
-- signal there is for telling a deliberate mapping from an inherited one.

-- +goose Down
DROP TABLE application_field_mappings;
