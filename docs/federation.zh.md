...
或 Java 应用往往用 CAS。**三者用相同的名字回答相同的事实**——租户、角色、机构——所以
一个从其中一种迁到另一种的应用，看到的信息不会有任何变化。
...
三件事会结束一个会话：退出登录、改密码、管理员停用账号。每一件都会：

- 立即作废每一个 Portico 会话令牌，并且
- 撤销每一个依赖方持有的 refresh token。
