...
每种协议都有自己的一套：[字段名称对照](#字段名称对照)就是用来对照的那张表。
...
第三种给法有个用之前该知道的后果：本服务器发送的 `Content-Security-Policy` 允许
`self`、`data:`、`https:` 三种来源的图片，所以**完整的 `https` 地址会正常渲染**。
纯 `http` 地址在注册时仍会被接受——留给公网证书覆盖不到的内网应用用——但**不会渲染**：
策略放行的是 `https:`，不是 `http:`。前两种都是同源，两种情况下都不受影响。
...
钥，以及怎么校验回来的东西。这两家没有这份文档，所以各自是**编译进服务端的一个适
配器** —— 也因此，多支持一家是一次发版，而不是一条配置。

先选类型，表单会跟着变。

| | |
|---|---|
| **微信** | 开放平台的**网站应用** —— 扫码那种。不是公众号网页授权（只能在微信自带浏览器里用，登不了控制台），也不是企业微信（那是另一个产品）。填的是 **AppID** 和 **AppSecret**，就按这个名字，因为微信自己的控制台就是这么叫的。 |
| **钉钉** | 它 2022 年起的那套登录 API。`scope=openid` 是钉钉的写法，不是承诺：**回来的东西没有任何签名**。 |

**两者都没有 issuer 这一栏**，这是刻意的而不是省事。issuer 是所有已存 subject 所
在的命名空间 —— 这里的身份是 `(issuer, subject)` 这一对 —— 所以它由服务端定死。
如果两个租户能各填各的，同一个人就会变成两个身份。

**保存时也不做校验。** OIDC 的 issuer 在写库之前会被连一次，这正是让写错的地址在
表单上就失败的原因；而对这两家没有等价的问题可问 —— 可能出错的是凭据，而两家都没
有「不让人真的登一次就能验证凭据」的接口。**所以对它们，第一次登录就是那次测试。**

#### 功能限制

值得直说，因为它们和上面那些提供方共用同一个按钮、同一个回调：

- **没有 ID 令牌。** 整个交换里没有任何东西是签名的。所以换取令牌的请求是**服务
  端对服务端**、走 TLS、带 client secret，而浏览器只被信任传递一个授权码。
- **没有 nonce**，因为没有令牌可以携带它。
- **没有 PKCE**，两家都不实现。

把回调和「本服务器发起过的那次请求」绑在一起的，只剩那个一次性的 `state` —— 而一
个 OIDC 提供方要过三道独立的检查。**这是那两家提供的东西本来就是这样，不是这里放
弃了什么。**

**微信一个邮箱地址都不返回**，所以上面那个开关对它做不了任何事，表单里也不出现。
钉钉会返回一个，但不声称它被验证过，Portico 因此按「未验证」记录。

#### 前置配置建议

微信用 `unionid` 标识一个人 —— 前提是应用绑定了开放平台账号；没绑就只有 `openid`。
Portico 优先用前者，**并把用了哪个记在行里**，因为这个区别以后会咬人：一个应用如果
**后来**才拿到 unionid，所有按 openid 绑定的身份都会失配，那些人会被告知「账号未关
联」。那是一次迁移，而它写得出来正是因为行里说明了用的是哪个键 —— 但**在有人关联之
前就把应用绑好开放平台，比事后便宜得多**。
...
三件事会结束一个会话：退出登录、改密码、管理员停用账号。三者都会**立即吊销每一个
...
**"退出 Portico 时一并吊销依赖方的刷新令牌"是一个选择，不是一项义务。** 不动它
...
——合法持有者手上会是那个替换件——所以处理方式是**吊销整条链**而不是让这一次调用失败，
因为**哪一环泄露的是无法知道的**。
...
| SAML | **没有东西可吊销**——不存在服务端会话，因为没有单点登出。服务提供方自己的会话完全不受影响，按它自己的规则结束。 |
| CAS | **没有东西可吊销**——一张票只活一分钟且一次性，而且没有 ticket-granting ticket。某个服务自己的会话与 SAML 一样，是它自己的事。 |
| 外部身份提供方 | **没有东西可吊销，而且什么也到不了对方。** 退出 Portico 不会把任何人从 Google 或 Entra 退出去；下一次点那个按钮会立刻重新登进来，因为对方仍然持有它自己的会话。这不是这里的缺口——**这就是这个方向上的联邦是什么意思**，和上面两行是同一件事，只是换了一侧看。 |
...
- **过期的刷新令牌会在过期后再多活三十天。** 每小时的清理只有在一条轮换链里的
  ...
  发整条链的吊销，而这正是一个被盗的刷新令牌被抓住的方式。**那一行是证据，所以
  它比凭据活得久。**
- **访问令牌无法被吊销。** 见上；`/revoke` 端点会接受它们并按 RFC 7009 的要求成功应答，
  但真正被吊销的只有刷新令牌。
- **没有授权确认页**，如上所述。
...
出登录、改密码、停用时**第三个需要吊销的东西**。搭在既有会话上，意味着那三件事已经把
它覆盖了。
...
会用一个拥有全部事实的账号验证三种协议，把回来的名字与这里写的比对——**双向比对，
...
有三处细节决定它能否一次验证通过，而**每一处出错时看起来都像是别的问题**：
