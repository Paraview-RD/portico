...
// Branding and audit logs used to be sections of settings/, reached by an
// anchor that carried one language's id into the other's build and landed
// on the page top instead of the section — twice. Each is its own page
// now, so BrandingPage's and AuditLogsPage's "view docs" links need
// nothing more than the page to exist in both languages: no anchor, so no
// id to go stale.
func TestBrandingAndAuditLogsAreTheirOwnPagesInBothLanguages(t *testing.T) {
	if !docs.Available() {
		t.Skip("manual not built; run ./hack/build-docs.sh")
	}

	for _, path := range []string{
		docs.Prefix + "/branding/",
		docs.Prefix + "/zh/branding/",
		docs.Prefix + "/audit-logs/",
		docs.Prefix + "/zh/audit-logs/",
	} {
		rec := get(t, path)
		if rec.Code != http.StatusOK {
