...
var publicAPIRoutes = map[string]string{
	...
	"GET /api/v1/landing": "what the root address does, which the console has to " +
		"know before it renders anything for somebody who is not signed in — and " +
		"which is the same for every visitor, signed in or not",
...
var selfServiceAPIRoutes = map[string]string{
	...
	"GET /api/v1/users/me/external-identities":             "the providers you have linked to your own account",
	"POST /api/v1/users/me/external-identities/{id}/start": "links one to your own account; the account comes from the session, never from the request",
	"DELETE /api/v1/users/me/external-identities/{id}":     "unlinks one of your own",
