...
func TestOpenIsIdempotent(t *testing.T) {
	...
	first, err := store.Open("sqlite", dsn)
	if err != nil {
	...
	second, err := store.Open("sqlite", dsn)
	if err != nil {
...
func TestOpenRejectsUnknownDriver(t *testing.T) {
	if _, err := store.Open("postgres", "whatever"); err == nil {
...
// A value written and read back must compare equal, which is what lets
// service code treat these as ordinary timestamps.
func TestDBTimeSurvivesSecondPrecision(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()

	// A time with sub-second precision is truncated on the way in; reading
	// it back must match the truncated value, not the original.
	now := store.Now()
	...
	if !got.UpdatedAt.Equal(now.Time) {
