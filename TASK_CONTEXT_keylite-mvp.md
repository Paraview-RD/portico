...
| # | 模块 | 需求章节 | 状态 |
|---|---|---|---|
| 0 | 环境与骨架（Go 工具链 / 目录 / 配置 / 日志 / 健康检查） | — | ✅ `45a2842` |
| 1 | 数据库 schema + migration | §3.1/3.4 | ✅ `be8fb25` |
| 2 | 用户账号管理（增改查/启禁用/改密/个人中心） | §3.1 | ✅ `0e94328` |
| 3 | Excel 批量导入 | §3.1 | ✅ `1180035` |
| 4 | 自主注册 + 注册开关 | §3.1/3.10 | ✅ `0e94328` |
| 5 | 登录 / 会话 / JWT 签发校验 | §3.2/3.6 | ✅ `0e94328` |
| 6 | 两档角色 + 全局鉴权拦截 | §3.3/3.5 | ✅ `0e94328` |
| 7 | 一级机构管理 + 用户机构归属 | §3.4 | ✅ `0e94328` |
| 8 | 开放鉴权接口（用户信息 / 身份校验） | §3.7 | ✅ `0e94328` |
| 9 | 下游同步接口（幂等拉取） | §3.8 | ✅ 无需新端点，`/users/me` 即满足 §3.8.2 全部字段 |
| 10 | 日志审计（登录/操作/鉴权/注册/机构变更） | §3.9 | ✅ `0e94328` |
| 11 | 系统配置（会话超时/Token 有效期/注册开关/初始管理员） | §3.10 | ✅ `0e94328` |
| 12 | 前端页面（登录/注册/用户/机构/日志/配置/个人中心） | §3.5 菜单自适应 | 🔄 进行中 |
| 13 | go:embed 集成 + Dockerfile + compose | — | ⬜ |
| 14 | 文档收尾（README/access-guide/integrations/CHANGELOG） | — | ⬜ |

## 后端已完成要点（前端对接必读）

- **API 前缀** `/api/v1`，信封 `{code, message, data}`，成功 `code="SUCCESS"`；失败 HTTP 状态与 code 一致（不存在 200+错误码）
- **鉴权** `Authorization: Bearer <jwt>`；401 的 code 有 `MISSING_TOKEN`/`INVALID_TOKEN`/`TOKEN_EXPIRED`/`TOKEN_REVOKED`/`ACCOUNT_DISABLED`，前端须统一清 token 跳登录
- **⚠️ 改密即失效**：`POST /users/me/password` 成功后当前 token 立即作废（token_version 自增），响应含 `reauthenticationRequired: true`，前端必须登出跳登录，不能只弹"保存成功"
- **登出同理**：`POST /auth/logout` 后当前 token 立即作废
- **菜单按角色**：读 `GET /users/me` 的 `role`（`SUPER_ADMIN`/`USER`）；前端隐藏只是装饰，真正拦截在后端 `RequireAdmin`
- **端点清单**：见 `docs/api-conventions.md` 与 `internal/server/routes.go`
- **测试**：197+ 用例全绿；两处安全分支做过变异测试确认可捕获回归
