...
MFA，以及请求限流。这些东西的路线图在
[docs/requirements/v0.1-requirements.md](docs/requirements/v0.1-requirements.md)（英文）。

> **在把它暴露到网络之前：** Portico 提供的是明文 HTTP，也不对请求限流，两者都
> 是刻意的。账号在多次登录失败后确实会锁定，但那是另一种控制：它阻止的是某一个
> 账号的密码被猜出来，对大量尝试给服务器带来的负载毫无作用。它必须跑在一个终结
> TLS、并对 `/api/v1/auth/*` 限流的反向代理后面 —— 原因见
