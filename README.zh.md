...
<https://demo.portico-dental.com> —— 一个公开演示环境，任何人都可以申请一个属
于自己的租户：填一个邮箱地址、打开链接，就会得到一个租户，里面预置了一个行业的
示例人员、组织与应用。

在做判断之前有两件事值得先知道。它跑在免费实例上，安静十五分钟就会休眠，所以隔
一段时间后的第一个请求要等大约五十秒的冷启动 —— 那是托管方的限制，不是软件的。
另外，你申请到的租户有 **14 天**的期限：到期后是**停用而不是删除**，再过一周才真正
删掉 —— 那一步才把它的名字和名额还给下一位访客。所以不要往里放任何真实的东西；如果
还想接着看，**在它失效之前**提出来，而不是之后。
